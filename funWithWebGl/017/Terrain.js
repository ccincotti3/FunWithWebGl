class Terrain {
  static createModel(gl, keepRawData) {
    return new Modal(Terrain.createMesh(gl, 10, 10, 20, 20, keepRawData))
  }
  ...
  static createMesh(gl, w, h, rLen, cLen, keepRawData) {
    ...
    //Perlin Noise
    noise.seed(1);
    var perlinH = 0,					//temporary height
    ...
    for(var i=0; i < vLen; i++){
      ...
      perlinH = noise.perlin2((cRow+1)/freq, (cCol+1)/freq) * maxHeight;

      //Create Vertices,x,y,z
      aVert.push(cStart+cCol*cInc, 0.2 + perlinH, rStart+cRow*rInc);
